...
`tools/list` at `https://gateway.pipeworx.io/mathjs/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/evaluate \
  -H 'Content-Type: application/json' \
  -d '{"expression":"2 + 3 * 4"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/evaluate`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
